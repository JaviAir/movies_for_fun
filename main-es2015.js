...
let AppComponent = class AppComponent {
    ...
    ngOnInit() {
        ...
        this.spinnerSub = this.movieService.spinnerSubject.subscribe((bool) => {
...
/* harmony import */ var ngx_infinite_scroll__WEBPACK_IMPORTED_MODULE_17__ = __webpack_require__(/*! ngx-infinite-scroll */ "./node_modules/ngx-infinite-scroll/modules/ngx-infinite-scroll.js");
...
let HomeComponent = class HomeComponent {
    ...
    ngOnInit() {
        ...
        // subscribe to spinnerSubject<boolean>
        this.spinnerSub = this.movieService.scrollSpinnerSubject.subscribe((bool) => {
            this.showScrollSpinner = bool;
    ...
    onRefresh() {
        ...
        const urlGenre = this.selectedGenreName;
        const urlLetter = this.selectedCharName;
        ...
        this.genre.forEach((element, i) => {
            if (element === urlGenre) {
                this.movieService.selectedGenreIndex = i;
                this.selectedGenreIndex = i;
            }
            else if (this.movieService.selectedGenreIndex === 0 && urlGenre !== 'All' && i === element.length - 1) {
        ...
        this.alphabet.forEach((element, i) => {
            if (element === urlLetter) {
                this.movieService.selectedCharIndex = i;
                this.selectedCharIndex = i;
            }
            else if (this.selectedCharIndex === 0 && urlLetter !== 'All' && i === element.length - 1) {
                console.log('This genre does not exist!');
            }
        }); // this is for the disabled porperty and alphabet indexes to work correctly
    }
    onLetterSelected(charIndex) {
...
let MovieDetailComponent = class MovieDetailComponent {
    constructor(route, movieService, _location, modalService) {
        this.route = route;
        this.movieService = movieService;
...
let MovieEditComponent = class MovieEditComponent {
    constructor(route, router, movieService, _location, modalService) {
        this.route = route;
        this.router = router;
        this.movieService = movieService;
        this._location = _location;
    ...
    ngOnInit() {
        this.movieSubjectSubscription = this.movieService.movieSubject.subscribe((movie) => {
            this.movie = movie;
            if (this.movie) {
                this.initForm();
            }
...
let MovieItemComponent = class MovieItemComponent {
  ...
  My4zaC0zNGwxNy00MC45IDE3IDQwLjl6IiAvPgogIDwvc3ZnPg==`;
    ...
    convertSpaceToUnderscore(title) {
...
let MovieListComponent = class MovieListComponent {
    ...
    ngOnInit() {
        this.movieArraySubscription = this.movieService.movieArrayUpdated.subscribe((movies) => {
            this.movieArray = movies;
        });
        this.movieService.readmovieArray();
        this.movieService.emptyArraySubject.subscribe((data) => {
            this.noMoviesinDB = data;
        });
        this.spinnerSub = this.movieService.spinnerSubject.subscribe((data) => {
...
class Movie {
    constructor(_id, title, description, image, genre, rating) {
        this._id = _id;
        this.title = title;
...
let MovieDetailResolver = class MovieDetailResolver {
...
let MovieListResolver = class MovieListResolver {
    constructor(movieService) {
        this.movieService = movieService;
    }
    resolve(route) {
...
let HttpService = class HttpService {
    ...
    getMovies(n) {
        return this.http.post(this.apiUrl + 'pagemovies', {
            n: n
        });
    }
    getMovieInstance(urlTitle) {
        return this.http.get(this.apiUrl + 'movies/' + urlTitle);
    }
    addMovie(movie) {
        return this.http.post(this.apiUrl + 'movies', movie);
    }
    updateMovie(oldTitle, movie) {
...
let MoviesService = class MoviesService {
    ...
    getMovieInstance(urlTitle) {
        ...
        this.httpService.getMovieInstance(urlTitle).subscribe((movie) => {
            // console.log(movie);
            this.movieSubject.next(movie);
            // this.movieParam = movie;
            // return this.movieParam;
    ...
    oddOneOut(currentGenre, currentLetter) {
    ...
    deleteMovie(movieId) {
        this.httpService.deleteMovie(movieId).subscribe(() => {
            this.movies.splice(this.currentMovieIndex, 1);
            this.readmovieArray();
        });
    }
    ...
    filterAlphabetically(selectedChar, loadMore) {
    ...
    filterGenreAndAlphabetical(selectedCharName, selectedGenreName, loadMore) {
